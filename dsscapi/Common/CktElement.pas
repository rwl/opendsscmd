...
{$ENDIF}
    ...
    PUBLIC
      ...
      {Total Noderef array for element}
        ...
        procedure GetCurrents(Curr: pComplexArray; ActorID: Integer); VIRTUAL; //Get present value of terminal Curr for reports
        procedure GetInjCurrents(Curr: pComplexArray; ActorID: Integer); VIRTUAL;   // Returns Injextion currents
        function InjCurrents(ActorID: Integer): Integer; VIRTUAL; // Applies to PC Elements Puts straight into Solution Array
        ...
        procedure RecalcElementData(ActorID: Integer); VIRTUAL;
        procedure CalcYPrim(ActorID: Integer); VIRTUAL;
...
// Check for an almost certain programming error
    ...
    if Value <> Fnconds then
        ActiveCircuit[ActiveActor].BusNameRedefined := TRUE;
    Fnconds := Value;
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
procedure TDSSCktElement.GetCurrents(Curr: pComplexArray; ActorID: Integer);  //Get present value of terminal Curr for reports
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
procedure TDSSCktElement.GetInjCurrents(Curr: pComplexArray; ActorID: Integer);
...
procedure TDSSCktElement.GetLosses(var TotalLosses, LoadLosses,
    NoLoadLosses: Complex; ActorID: Integer);
begin
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
function TDSSCktElement.InjCurrents(ActorID: Integer): Integer;  // Applies to PC Elements
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
procedure TDSSCktElement.RecalcElementData(ActorID: Integer);
begin
    DoSimpleMsg('Virtual proc RecalcElementData in Base CktElement Class Called for Device = "' + Name + '"', 754);
end;

//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
procedure TDSSCktElement.CalcYPrim(ActorID: Integer);
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
procedure TDSSCktElement.ComputeIterminal(ActorID: Integer);
begin
...
procedure TDSSCktElement.GetSeqLosses(var PosSeqLosses, NegSeqLosses, ZeroModeLosses: complex; ActorID: Integer);
begin
...
procedure TDSSCktElement.MakePosSequence(ActorID: Integer);
var
    i: Integer;
    grnd: Boolean;
begin
...
procedure TDSSCktElement.ComputeVterminal(ActorID: Integer);
