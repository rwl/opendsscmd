...
begin
    ...
    if count > 0 then
    begin
        Result := Format('[%g', [dblarray^[1]]);
        for i := 2 to count do
            Result := Result + Format(', %g', [dblarray^[i]]);
        Result := Result + ']';
    end;
...
begin
    ...
    if count > 0 then
    begin
        Result := Format('[%g +j %g', [cpxarray^[1].re, cpxarray^[1].im]);
        for i := 2 to count do
            Result := Result + Format(', %g +j %g', [cpxarray^[i].re, cpxarray^[i].im]);
        Result := Result + ']';
    end;
...
// Invalidate All PC Elements; Any could be a machine
    ...
    begin
        ...
        begin
            if pcelem.Enabled then
                pcelem.YprimInvalid[ActiveActor] := TRUE;
            pcelem := PCElements.Next;
        end;
    end;
end;
...
begin
    if NodeRef = 0 then
        ...
        Result := ActiveCircuit[ActiveActor].MapNodeToBus^[NodeRef].NodeNum
end;
...
begin
    ...
    begin
        ...
        for j := 1 to ThisElement.NPhases do
            if ThisElement.Closed[j, ActiveActor] then
            begin
                Result := TRUE;
                Break;
            end;
...
begin
    ...
    try
       ...
       // something else
            if TDSSCktElement(ActiveDSSObject[ActiveActor]).HasBeenSaved then
                Continue;
...
begin
    ...
    try
       ...
       // something else
        ...
        if Nrecords > 0 then
            SavedFileList[ActiveActor].Add(FileName)
        else
            DeleteFile(FileName);
  ...
  //  Write(F, NeworEdit, ' "', ParClass.Name + '.' + ActiveDSSObject[ActiveActor].Name,'"');
   ...
   // Handle disabled circuit elements;   Modified to allow applets to save disabled elements 12-28-06
    if (ActiveDSSObject[ActiveActor].DSSObjType and ClassMask) <> DSS_Object then
        if not TDSSCktElement(ActiveDSSObject[ActiveActor]).Enabled then
            Write(F, ' ENABLED=NO');
...
begin
    ...
    if iBusidx > 0 then
        while ActiveCircuit[ActiveActor].Buses^[iBusidx].FindIdx(Result) <> 0 do
            Inc(Result);
...
{Make sure buslist exists}
    ...
    if ActiveCircuit[ActiveActor] = NIL then
        Exit;
    if ActiveCircuit[ActiveActor].BusList.ListSize <= 0 then
        Exit;
    with ActiveCircuit[ActiveActor] do
    begin
