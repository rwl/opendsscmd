...
// = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
    ...
    PUBLIC
        ...
        procedure CalcYPrim(ActorID: Integer); OVERRIDE;    // Always Zero for a CapControl
...
{--------------------------------------------------------------------------}
function TCapControl.Edit(ActorID: Integer): Integer;
var
...
begin
  ...
  // continue parsing WITH contents of Parser
    ...
    ActiveCircuit[ActorID].ActiveCktElement := ActiveCapControlObj;
    ...
    begin
        ...
        begin
            ...
            case ParamPointer of
                ...
                5:
                    ControlVars.PTRatio := Parser[ActorID].DblValue;
                6:
                    ControlVars.CTRatio := Parser[ActorID].DblValue;
                7:
                    ControlVars.ON_Value := Parser[ActorID].DblValue;
                8:
                    ControlVars.OFF_Value := Parser[ActorID].DblValue;
                9:
                    ControlVars.ONDelay := Parser[ActorID].DblValue;
                10:
                    ControlVars.Voverride := InterpretYesNo(param);
                11:
                    ControlVars.Vmax := Parser[ActorID].DblValue;
                12:
                    ControlVars.Vmin := Parser[ActorID].DblValue;
                13:
                    ControlVars.OFFDelay := Parser[ActorID].DblValue;
                14:
                    ControlVars.DeadTime := Parser[ActorID].DblValue;
                15:
                    ...
                    if CompareTextShortest(param, 'min') = 0 then
                        ...
                        ControlVars.FCTPhase := max(1, Parser[ActorID].IntValue);
                16:
                    if CompareTextShortest(param, 'avg') = 0 then
                    ...
                    if CompareTextShortest(param, 'min') = 0 then
                        ...
                        ControlVars.FPTPhase := max(1, Parser[ActorID].IntValue);
                17:
                begin
                ...
                19:
                    UserModel.Name := Parser[ActorID].StrValue;  // Connect to user written model
                20:
                    if UserModel.Exists then
                        UserModel.Edit := Parser[ActorID].StrValue;  // Send edit string to user model
                21:
                    FpctMinKvar := Parser[ActorID].DblValue;
                22:
...
procedure TCapControlObj.MakePosSequence(ActorID: Integer);
begin
...
{--------------------------------------------------------------------------}
procedure TCapControlObj.CalcYPrim(ActorID: Integer);
begin
...
procedure TCapControlObj.GetCurrents(Curr: pComplexArray; ActorID: Integer);
var
    i: Integer;
begin
...
procedure TCapControlObj.GetInjCurrents(Curr: pComplexArray; ActorID: Integer);
var
    i: Integer;
...
{--------------------------------------------------------------------------}
procedure TCapControlObj.DoPendingAction(const Code, ProxyHdl: Integer; ActorID: Integer);
            ...
            CTRL_OPEN:
                case ControlledCapacitor.NumSteps of
                    ...
                    begin
                        ...
                        begin
                            ...
                            if ShowEventLog then
                                AppendtoEventLog('Capacitor.' + ControlledElement.Name, '**Opened**', ActorID);
                    ...
                    begin      // Do this only if at least one step is closed
                        if not ControlledCapacitor.SubtractStep(ActorID) then
                        begin
                            PresentState := CTRL_OPEN;
                            ControlledElement.Closed[0, ActorID] := FALSE;   // Open all phases of active terminal
                            if ShowEventLog then
                        ...
                        if ShowEventLog then
                            AppendtoEventLog('Capacitor.' + ControlledElement.Name, '**Step Down**', ActorID);
                    end;
                end;
            CTRL_CLOSE:
            begin
                ...
                begin
                    ...
                    if ShowEventLog then
                        AppendtoEventLog('Capacitor.' + ControlledElement.Name, '**Closed**', ActorID);
                    PresentState := CTRL_CLOSE;
                ...
                begin
                    if ControlledCapacitor.AddStep(ActorID) then
                        if ShowEventLog then
                            AppendtoEventLog('Capacitor.' + ControlledElement.Name, '**Step Up**', ActorID);
                end;
            end;
        else
...
begin
                ...
                case PresentState of
                    CTRL_OPEN:
                        ...
                        begin
                            ...
                            VoverrideEvent := TRUE;
                            if ShowEventLog then
                                AppendtoEventLog('Capacitor.' + ControlledElement.Name, Format('Low Voltage Override: %.8g V', [Vtest]), ActorID);
                        end;
                    CTRL_CLOSE:
                        if Vtest > Vmax then
                        begin
                            ...
                            VoverrideEvent := TRUE;
                            if ShowEventLog then
                                AppendtoEventLog('Capacitor.' + ControlledElement.Name, Format('High Voltage Override: %.8g V', [Vtest]), ActorID);
                        end;
                end;
    ...
    begin
        ...
        begin
            ...
            Armed := TRUE;
            if ShowEventLog then
                AppendtoEventLog('Capacitor.' + ControlledElement.Name, Format('**Armed**, Delay= %.5g sec', [TimeDelay]), ActorID);
        end;
        ...
        begin
            ...
            Armed := FALSE;
            if ShowEventLog then
                AppendtoEventLog('Capacitor.' + ControlledElement.Name, '**Reset**', ActorID);
        end;
    end;  {With}
end;
