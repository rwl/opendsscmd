    ...
    begin
        ...
        begin
            ...
            case ParamPointer of
                ...
                1:
                    ElementName := 'Transformer.' + lowercase(param);
                2:
                    ElementTerminal := Parser[ActorID].IntValue;
                3:
                    Vreg := Parser[ActorID].DblValue;
                4:
                    Bandwidth := Parser[ActorID].DblValue;
                5:
                    PTRatio := Parser[ActorID].DblValue;
                6:
                    CTRating := Parser[ActorID].DblValue;
                7:
                ...
                12:
                    revVreg := Parser[ActorID].DblValue;
                13:
                    revBandwidth := Parser[ActorID].DblValue;
                14:
                    revR := Parser[ActorID].DblValue;
                15:
                    revX := Parser[ActorID].DblValue;
                16:
                    TapDelay := Parser[ActorID].DblValue;
                17:
                    DebugTrace := InterpretYesNo(Param);
                18:
                    TapLimitPerChange := max(0, Parser[ActorID].IntValue);
                19:
                    FInversetime := InterpretYesNo(Param);
                20:
                    TapWinding := Parser[ActorID].intValue;
                21:
                begin
                ...
                27:
                    RemotePTRatio := Parser[ActorID].DblValue;
                28:
                    TapNum := Parser[ActorID].IntValue;
                29:
                ...
                30:
                    LDC_Z := Parser[ActorID].DblValue;
                31:
                    revLDC_Z := Parser[ActorID].DblValue;
                32:
                        ...
                        case ControlMode of
                            ...
                            begin
                                ...
                                if ShowEventLog then
                                    AppendtoEventLog('Regulator.' + ControlledElement.Name, Format(' Changed %d taps to %-.6g.', [Lastchange[ActorID], PresentTap[TapWinding, ActorID]]), ActorID);
                            ...
                            begin
                                ...
                                if PendingTapChange <> 0.0 then
                                    ControlQueue.Push(DynaVars.intHour, Dynavars.t + TapDelay, 0, 0, Self, ActorID)
                                else
                                    Armed := FALSE;
                            end;
                            ...
                            begin
                                ...
                                PresentTap[TapWinding, ActorID] := PresentTap[TapWinding, ActorID] + TapChangeToMake;
                                if ShowEventLog then
                                    AppendtoEventLog('Regulator.' + ControlledElement.Name, Format(' Changed %d tap to %-.6g.', [Lastchange[ActorID], PresentTap[TapWinding, ActorID]]), ActorID);
                                if (DebugTrace) then
                                    RegWriteDebugRecord(Format('--- Regulator.%s Changed %d tap to %-.6g.', [ControlledElement.Name, Lastchange[ActorID], PresentTap[TapWinding, ActorID]]));
