...
{$ENDIF}
    ...
    PUBLIC
        ...
        function GetWindingCurrentsResult(ActorID: Integer): String;  // All winding currents in string


        procedure MakePosSequence(ActorID: Integer); OVERRIDE;  // Make a positive Sequence Model

        property PresentTap[i, ActorID: Integer]: Double READ Get_PresentTap WRITE Set_PresentTap;
...
procedure TTransfObj.GetAllWindingCurrents(CurrBuffer: pComplexArray; ActorID: Integer);
...
function TTransfObj.GetWindingCurrentsResult(ActorID: Integer): String;
  ...
  {First, determine if we can convert this one.}
    ...
    begin //disable if any terminal not connected to phase one
       ...
       {Load up auxiliary parser}
            AuxParser[ActiveActor].CmdString := GetBus(iW);
            AuxParser[ActiveActor].NextParam;
            S := AuxParser[ActiveActor].ParseAsBusName(N, @Nodes, ActorID);
            if N = 0 then
                OnPhase1 := TRUE;
            for i := 1 to N do
                if Nodes[i] = 1 then
...
{           SetElement(j, j, CmulReal_im(GetElement(j, j) , ppm_FloatFactorPlusOne));}
 ...
 {$IFDEF TRANSDEBUG}
    AssignFile(F, CircuitName_[ActiveActor] + 'Transformer_' + Name + '.TXT');
    Rewrite(F);
...
begin
    ...
    if ActiveCircuit[ActiveActor].Solution.Frequency < 0.51 then
