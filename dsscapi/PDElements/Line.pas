...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
function TLine.Edit(ActorID: Integer): Integer;
...
begin
  ...
  // continue parsing with contents of Parser
    ...
    ActiveCircuit[ActorID].ActiveCktElement := ActiveLineObj;  // use property to set this value
...
{Nphases: See below};
                ...
                7:
                    x1 := Parser[ActorID].Dblvalue;
                8:
                    r0 := Parser[ActorID].Dblvalue;
                9:
                    x0 := Parser[ActorID].Dblvalue;
                10:
                begin
                    c1 := Parser[ActorID].Dblvalue * 1.0e-9;
                    FCapSpecified := TRUE;
                end; // Convert from nano to farads
                11:
                begin
                    c0 := Parser[ActorID].Dblvalue * 1.0e-9;
                    FCapSpecified := TRUE;
                end;
                12:
                    DoRmatrix(ActorID);
                ...
                26:
                begin
                    c1 := Parser[ActorID].Dblvalue / (twopi * BaseFrequency) * 1.0e-6;
                    FCapSpecified := TRUE;
                end;
                27:
                begin
                    c0 := Parser[ActorID].Dblvalue / (twopi * BaseFrequency) * 1.0e-6;
                    FCapSpecified := TRUE;
                end;
                28:
                begin
...
begin
    ...
    begin
        ...
        begin
            ...
            FMakeZFromGeometry(ActiveCircuit[ActorID].Solution.Frequency); // Includes length in proper units
        ...
        begin
            ...
            FMakeZFromSpacing(ActiveCircuit[ActorID].Solution.Frequency); // Includes length in proper units
            if SolutionAbort then
                Exit;
...
procedure TLineObj.GetLosses(var TotalLosses, LoadLosses, NoLoadLosses: Complex; ActorID: Integer);
begin
...
procedure TLineObj.GetSeqLosses(var PosSeqLosses, NegSeqLosses, ZeroSeqLosses: complex; ActorID: Integer);
...
procedure TLineObj.MakePosSequence(ActorID: Integer);
var
    S: String;
    C1_new, Cs, Cm: Double;
    Z1, ZS, Zm: Complex;
      ...
      // Redefine property values to make it appear that line was defined this way originally using matrices
        ...
        begin
           ...
           // Find the bus in common between the two lines
            ...
            begin
                TestBusNum := ActiveCircuit[ActiveActor].MapNodeToBus^[NodeRef^[1 + (i - 1) * Fnconds]].BusRef;
                for j := 1 to 2 do
                begin
                    if ActiveCircuit[ActiveActor].MapNodeToBus^[OtherLine.NodeRef^[1 + (j - 1) * OtherLine.Nconds]].BusRef = TestBusNum then
                    begin
                        Common1 := i;
                        Common2 := j;
                        Break;
...
begin
    ...
    pControlElem := ActiveCircuit[ActiveActor].DSSControls.First;
